...
import { Upload, CheckCircle, Settings2 } from "lucide-react"
...
export default function GateOutPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Validate if files are needed? (Optional for now, but usually gate pass is required)
    // if (!gateOutData.gatePassFile) {
    //     toast({ title: "Validation Error", description: "Gate Pass is required.", variant: "destructive" })
    //     return
    // }
        ...
        <DialogContent className="max-w-[95vw]! w-full max-h-[95vh] overflow-y-auto p-0">
          ...
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-slate-900">
              Gate Out - {selectedGroup?.doNumber}
            </DialogTitle>
          </DialogHeader>
