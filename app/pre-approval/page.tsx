...
export default function PreApprovalPage() {
  ...
  const [skuSellingPrices, setSkuSellingPrices] = useState<any[]>([])

  // Helper for fuzzy matching SKU names with packing material
  const findPricingForSku = (skuName: string) => {
    if (!skuName || !skuSellingPrices.length) return null;
    ...
    const normalizedSku = normalize(skuName);

    // Find the most specific match (longest normalized packing material that matches)
    return skuSellingPrices
      .filter(sp => {
        const normalizedPM = normalize(sp.packing_material);
  ...
  useEffect(() => {
    const fetchSellingPrices = async () => {
      try {
        const response = await skuSellingPriceApi.getAll()
        if (response.success && Array.isArray(response.data)) {
          setSkuSellingPrices(response.data)
        }
      } catch (error) {
        console.error("Failed to fetch SKU selling prices:", error)
      }
    }
    ...
    return formulas.map((item) => {
      ...
      try {
        if (!formula || formula === "") {
          ...
          calculatedRate = evaluateExpression(evalFormula)
        }
      } catch (error) {
    ...
    const fetchLatestVarCalc = async () => {
      try {
        ...
        if (response.success && response.data) {
          setLatestVarCalc(response.data);
        }
      } catch (error) {
        console.error("Failed to fetch latest variable parameters:", error);
      }
    };
    fetchLatestVarCalc();
  ...
  const handleApproveWithAdditions = async (itemsToApprove: any[]) => {
    ...
    try {
      ...
      for (const item of itemsToApprove) {
        ...
        if (dynamicCost && !dynamicCost.noVarCalc && dynamicCost.landingCost > 0) {
          ...
          if (enteredRate < minAllowed) {
            toast({
              title: "Rate Too Low",
              description: `Final rate for ${rateData?.skuName} is below landing cost (₹${minAllowed}).`,
              variant: "destructive"
            });
            setIsApproving(false);
            return;
      ...
      // Pre-calculate total new SKU budget per (DO + Oil Category) to avoid double-deduction
      const newSkuBudgetMap: Record<string, number> = {};
      itemsToApprove.forEach(item => {
        if (item._product._isNew) {
          const p = item._product;
          const cat = normalizeType(p.oilType || productRates[item._rowKey]?.productName || "");
          const key = `${p._baseDo}_${cat}`;
          newSkuBudgetMap[key] = (newSkuBudgetMap[key] || 0) + (parseFloat(productRates[item._rowKey]?.approvalQty || "0") || 0);
        }
      });

      console.log("[DEBUG] New SKU Budget Map:", newSkuBudgetMap);
                                                ...
                                                <TableCell className="p-2 min-w-[140px]">
                                                  <div className="relative">
                                                    ...
                                                    <Input
                                                      type="number"
                                                      className={cn(
                                                        "h-9 text-[10px] bg-white border-slate-200 font-medium",
                                                        ...
                                                        if (dynamicCost && !dynamicCost.noVarCalc && dynamicCost.landingCost > 0 && newValue !== "") {
                                                          ...
                                                          if (enteredRate < minAllowed) {
                                                            setRateValidationErrors({
                                                              ...
                                                              [rowKey]: `Below Landing Cost (Min ₹${minAllowed})`
                                                            });
                                                          } else if (enteredRate > maxAllowed) {
                                                            setRateValidationErrors({
                                                              ...rateValidationErrors,
                                                              [rowKey]: `Exceeds Max Margin (Max ₹${maxAllowed.toFixed(2)})`
                                                            });
                                                          } else {
